        ...
        <div className="space-y-6">
            ...
            <div className="bg-card border border-border rounded-xl p-6">
                <h3 className="font-semibold mb-4 flex items-center gap-2">
                    ...
                    ການສັ່ງການດ່ວນ (Quick Actions)
                </h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {[
                        {
                            action: "restart",
                            label: "ເລີ່ມເຄື່ອງໃໝ່",
                            icon: RefreshCw,
                        ...
                        <button
                            ...
                            onClick={() => {
                                if (
                                    ...
                                    if (
                                        !confirm(
                                            `ທ່ານແນ່ໃຈບໍ່ວ່າຕ້ອງການ ${a.label} ຄອມພິວເຕີນີ້?`,
                                        )
                                    )
                                        return;
                                }
            ...
            <div className="bg-card border border-border rounded-xl p-6">
                <h3 className="font-semibold mb-4 flex items-center gap-2">
                    ...
                    ການກວດສອບເຄືອຂ່າຍ (Network Diagnostics)
                </h3>
                <div className="flex gap-3 items-end">
                    ...
                    <button
                        onClick={() =>
                            sendCommand("traceroute", { host: pingHost })
                        }
                        disabled={actionLoading !== null}
                        className="px-4 py-2 bg-accent/20 text-accent border border-accent/30 rounded-lg text-sm hover:bg-accent/30 transition-colors"
                    >
            ...
            <div className="bg-card border border-border rounded-xl p-6">
                <h3 className="font-semibold mb-4 flex items-center gap-2">
                    ...
                    ປິດໂປຣເຊສ (Kill Process)
                </h3>
                <div className="flex gap-3 items-end">
                    <div className="flex-1">
                        <label className="text-xs text-muted mb-1 block">
                            ຊື່ໂປຣເຊສ (ເຊັ່ນ: notepad.exe) ຫຼື PID
                        </label>
                        <input
                            type="text"
                            value={killTarget}
                    ...
                    <button
                        onClick={() => {
                            ...
                            sendCommand(
                                "kill_process",
                                isNum
                                    ? { pid: parseInt(killTarget) }
                                    : { name: killTarget.trim() },
                            );
                            setKillTarget("");
                        }}
            ...
            <div className="bg-card border border-border rounded-xl p-6">
                <h3 className="font-semibold mb-4 flex items-center gap-2">
                    <Terminal className="w-4 h-4" />
                    ຣັນສະຄຣິບ PowerShell (Run PowerShell Script)
                </h3>
                <textarea
                    value={customScript}
                ...
                <div className="flex justify-end mt-3">
                    <button
                        onClick={() => {
                            ...
                            sendCommand("run_powershell", {
                                script: customScript.trim(),
                            });
                        }}
                        disabled={
                            actionLoading !== null || !customScript.trim()
                        }
                        className="px-4 py-2 bg-accent/20 text-accent border border-accent/30 rounded-lg text-sm hover:bg-accent/30 transition-colors"
                    >
                        ປະມວນຜົນ (Execute)
            ...
            <div className="bg-card border border-border rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="font-semibold flex items-center gap-2">
                        ...
                        ປະຫວັດການສັ່ງການ (Command History)
                    </h3>
                    <button
